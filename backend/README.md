...
Статус: backend-часть закрыта, кроме `docker-compose` (веха «День 8»). Готовы вехи
«Дни 1–2» (скелет, конфиг, миграции, клиент к Riot API), «Дни 3–4» (ограничитель
частоты, ретраи, кэш), «Дни 5–6» (репозитории, синхронизация, REST API) и «День 7»
(периодический sync worker, статистика, принудительная синхронизация).
